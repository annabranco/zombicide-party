...
import { getMediaQuery } from './devices';
...
export const calculateXpBar = (currentXp, highestXp, device) => {
  ...
  if (getMediaQuery() === DESKTOP) {
    xpLevels = Array.from({ length: orangeThreatThresold + 2 }, (_, i) => i);
  } else {
    if (currentXp <= blueThreatThresold) {
      if (device === MOBILE) {
        ...
        xpLevels.push('...');
        if (
          !xpLevels.includes(highestXp) &&
          highestXp <= yellowThreatThresold
        ) {
        ...
        if (
          !xpLevels.includes(highestXp) &&
          highestXp <= orangeThreatThresold
        ) {
          xpLevels.push(highestXp);
          xpLevels.push('...');
        }
